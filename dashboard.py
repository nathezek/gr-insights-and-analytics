...
if raw_data:
    ...
    with st.expander("📁 Uploaded Files Info", expanded=False):
        for key, df in raw_data.items():
            if df is not None:
                st.write(
                    f"**{key.title()}**: {len(df)                                          :,} rows × {len(df.columns)} columns"
                )
                ...
                if len(df) > max_telemetry_rows:
                    st.warning(f"⚠️ Will be sampled to {
                               max_telemetry_rows:,} rows")
    ...
    try:
        ...
        cleaned_telemetry = clean_telemetry(
            raw_data["telemetry"], max_rows=max_telemetry_rows, sample_frac=sample_frac
        )
        ...
        all_cleaned = join_datasets(
            raw_data, max_merged_rows=max_telemetry_rows)
    ...
    if len(all_cleaned) == 0:
        st.error(
            "❌ No valid telemetry data after cleaning (missing speed/lap/timestamp)"
        )
        st.stop()
    ...
    if "vehicle_number" in lap_data.columns:
        available_vehicles = sorted(
            lap_data["vehicle_number"].dropna().unique())
        if len(available_vehicles) > 1:
            selected_vehicle = st.sidebar.selectbox(
                "Select Vehicle", available_vehicles
            )
    ...
    if comparison_lap is not None and comparison_lap != selected_lap:
        ...
        if (
            "vehicle_number" in comparison_lap_data.columns
            and "vehicle_number" in lap_data.columns
        ):
            vehicle_nums = lap_data["vehicle_number"].dropna().unique()
            if len(vehicle_nums) == 1:
                comparison_lap_data = comparison_lap_data[
                    comparison_lap_data["vehicle_number"] == vehicle_nums[0]
                ]
    ...
    col3.metric("Memory", f"{lap_data.memory_usage(
        deep=True).sum() / 1024**2:.1f} MB")

    # Calculate lap time if available
    if "timestamp" in lap_data.columns:
        lap_duration = (
            lap_data["timestamp"].max() - lap_data["timestamp"].min()
        ).total_seconds()
    ...
    if st.button("🔮 Run AI Analysis", type="primary", use_container_width=True):
        if len(lap_data) > 100000:
            st.warning(
                f"⚠️ Large lap ({len(lap_data)                                 :,} points). This may take a moment..."
            )
        ...
        try:
            ...
            predicted_data, insights = predict_mistakes(
                model, lap_data, imputer, threshold_kmh=threshold, chunksize=chunk_size
            )
        ...
        with tab1:
            ...
            x_col = None
            if (
                "Laptrigger_lapdist_dls" in predicted_data.columns
                and predicted_data["Laptrigger_lapdist_dls"].notna().any()
            ):
            ...
            if len(plot_data) > 10000:
                st.caption(
                    f"📊 Displaying 10,000 of {
                        len(plot_data):,} points for performance"
                )
            ...
            for ch in available_channels:
                if ch == "brake":
                    ...
                    subplot_titles.append(
                        feature_name_mapping.get(ch, ch).title())
            ...
            if num_plots > 0:
                ...
                if "gear" in available_channels:
                    ...
                    fig.update_yaxes(
                        title_text=feature_name_mapping.get("gear", "Gear"),
                        row=row,
                        col=1,
                    )
                    row += 1
                ...
                if "aps" in available_channels:
                    ...
                    fig.update_yaxes(
                        title_text=feature_name_mapping.get(
                            "aps", "Throttle Position")
                        + " (%)",
                        row=row,
                        col=1,
                ...
                if "brake" in available_channels:
                    if "pbrake_f" in plot_data.columns:
                        fig.add_trace(
                            go.Scatter(
                                x=x_axis,
                                y=plot_data["pbrake_f"].values,
                                name=feature_name_mapping.get(
                                    "pbrake_f", "Brake Front"
                                ),
                    ...
                    fig.update_yaxes(
                        title_text="Brake Pressure", row=row, col=1)
                    row += 1
                ...
                if "steering" in available_channels:
                    fig.add_trace(
                        go.Scatter(
                            x=x_axis,
                            y=plot_data["Steering_Angle"].values,
                            name=feature_name_mapping.get(
                                "Steering_Angle", "Steering Angle"
                            ),
                ...
                fig.update_layout(
                    height=250 * num_plots, showlegend=True, hovermode="x unified"
                )

                st.plotly_chart(fig, use_container_width=True)
            else:
                st.warning(
                    "⚠️ No telemetry channels available for visualization")
        ...
        with tab2:
            ...
            if len(mistakes) > 0:
                mistake_x = (
                    mistakes["Laptrigger_lapdist_dls"].values
                    if "Laptrigger_lapdist_dls" in mistakes.columns
                    else mistakes.index.values
                )
                fig.add_trace(
                    go.Scatter(
                        x=mistake_x,
            ...
            too_slow = predicted_data[predicted_data["mistake_type"]
                                      == "TOO_SLOW"]
            too_fast = predicted_data[predicted_data["mistake_type"]
                                      == "TOO_FAST"]

            col1.metric(
                "Total Mistakes",
                f"{len(mistakes_all):,}",
                delta=f"{len(mistakes_all) / len(predicted_data)
                         * 100:.1f}% of lap",
            )
            ...
            if len(mistakes_all) > 0:
                ...
                worst_cols = ["speed", "predicted_speed",
                              "speed_error", "mistake_type"]
                if "Laptrigger_lapdist_dls" in mistakes_all.columns:
                    worst_cols.insert(0, "Laptrigger_lapdist_dls")

                worst = mistakes_all.nlargest(10, "abs_error")[
                    worst_cols].copy()
                ...
                if "Laptrigger_lapdist_dls" in worst.columns:
                    worst["Laptrigger_lapdist_dls"] = worst[
            ...
            try:
                if hasattr(model, "feature_importances_") and metadata is not None:
                    ...
                    if (
                        ...
                        friendly_names = [
                            feature_name_mapping.get(f, f) for f in feature_names
                        ]
                        ...
                        st.plotly_chart(
                            fig_importance, use_container_width=True)
                    else:
                        st.info(
                            "ℹ️ Feature names not available in model metadata")
                    ...
                    st.info(
                        "ℹ️ Model does not support feature importance extraction")
            except Exception as e:
        ...
        with tab3:
            ...
            if "Laptrigger_lapdist_dls" in predicted_data.columns:
                ...
                if len(sector_data) > 0:
                    ...
                    sector_data["sector"] = (
                        (sector_data["Laptrigger_lapdist_dls"] //
                         sector_length)
                        .fillna(0)
                        .astype(int)
                    )
                    sector_data["sector"] = sector_data["sector"].clip(
                        0, num_sectors - 1
                    )
                    ...
                    sector_stats.index = [
                        f"Sector {i + 1}" for i in sector_stats.index]
                    ...
                    with col1:
                        ...
                        fig_sector_error = px.bar(
                            ...
                            labels={"x": "Sector",
                                    "y": "Avg Speed Error (km/h)"},
                            color=sector_stats["Avg Speed Error (km/h)"],
                            color_continuous_scale="RdYlGn",
                            color_continuous_midpoint=0,
                        )
                        st.plotly_chart(fig_sector_error,
                    ...
                    with col2:
                        ...
                        st.plotly_chart(fig_sector_mistakes,
                                        use_container_width=True)

                    # Sector map
                    st.subheader("Track Map by Sector")
                    if (
                        "VBOX_Lat_Min" in sector_data.columns
                        and "VBOX_Long_Minutes" in sector_data.columns
                    ):
                        map_data = sector_data.dropna(
                            subset=["VBOX_Lat_Min", "VBOX_Long_Minutes"]
                        )
                        ...
                        if len(map_data) > 0:
                            fig_map = px.scatter_mapbox(
                                ...
                                color="sector",
                                hover_data=[
                                    "speed", "predicted_speed", "speed_error"],
                                zoom=13,
                                height=600,
                                color_continuous_scale="Viridis",
                            )
                    ...
                    st.warning(
                        "⚠️ No valid distance data after removing NaN values")
            else:
                st.warning(
                    "⚠️ Distance data not available for sector analysis")
        ...
        with tab4:
            ...
            if comparison_lap_data is not None and len(comparison_lap_data) > 0:
                ...
                if (
                    "Laptrigger_lapdist_dls" in predicted_data.columns
                    and "Laptrigger_lapdist_dls" in comparison_lap_data.columns
                ):
                    # Downsample both
                    if len(predicted_data) > 5000:
                        data1 = predicted_data.iloc[
                            :: len(predicted_data) // 5000
                        ].copy()
                    else:
                        data1 = predicted_data.copy()

                    if len(comparison_lap_data) > 5000:
                        data2 = comparison_lap_data.iloc[
                            :: len(comparison_lap_data) // 5000
                        ].copy()
                    else:
                    ...
                    data1 = data1.dropna(
                        subset=["Laptrigger_lapdist_dls", "speed"])
                    data2 = data2.dropna(
                        subset=["Laptrigger_lapdist_dls", "speed"])
                    ...
                    if len(data1) > 0 and len(data2) > 0:
                        ...
                        try:
                            ...
                            common_x = np.linspace(
                                max(
                                    data1["Laptrigger_lapdist_dls"].min(),
                                    data2["Laptrigger_lapdist_dls"].min(),
                                ),
                                min(
                                    data1["Laptrigger_lapdist_dls"].max(),
                                    data2["Laptrigger_lapdist_dls"].max(),
                                ),
                                1000,
                            )
                            ...
                            # Update axes
                            fig.update_xaxes(
                                title_text="Distance (m)", row=2, col=1)
                            fig.update_yaxes(
                                title_text="Speed (km/h)", row=1, col=1)
                            fig.update_yaxes(
                                title_text="Delta (km/h)", row=2, col=1)
                            ...
                            col1.metric("Average Delta", f"{
                                        avg_delta:.2f} km/h")
                            col2.metric("Max Advantage", f"{
                                        max_advantage:.2f} km/h")
                            col3.metric(
                                "Max Disadvantage", f"{
                                    max_disadvantage:.2f} km/h"
                            )
                            ...
                            fig.update_xaxes(
                                title_text="Distance (m)", row=1, col=1)
                            fig.update_yaxes(
                                title_text="Speed (km/h)", row=1, col=1)
                            fig.update_layout(
                        ...
                        st.warning(
                            "⚠️ No valid data after removing NaN values")
                ...
                st.info(
                    "ℹ️ Select a comparison lap in the sidebar to enable lap comparison"
                )
        ...
        with tab5:
            ...
            memory_size = predicted_data.memory_usage(
            ...
            with col1:
                ...
                speed_stats = predicted_data[[
                    "speed", "predicted_speed"]].describe()
                st.dataframe(speed_stats, use_container_width=True)

            with col2:
                st.write("**Error Statistics**")
                error_stats = predicted_data[[
                    "speed_error", "abs_error"]].describe()
            ...
            with st.expander("👀 Preview Data"):
                st.dataframe(predicted_data.head(100),
                             use_container_width=True)
