...
def engineer_features(df):
    ...
    if 'total_g' in df.columns and 'gear' in df.columns:
        engineered['g_per_gear'] = (df['total_g'] / (df['gear'] + 1).clip(1, None)).clip(0, 10)
...
def predict_mistakes(model, telemetry_df, imputer=None, threshold_kmh=10, chunksize=10000):
    """
    ...
    print(f"\n[INFO] Starting AI analysis on {len(telemetry_df):,} data points...")
    ...
    if 'speed' in result.columns:
        ...
        result.loc[result['speed_error'] < -threshold_kmh, 'mistake_type'] = 'TOO_SLOW'
        result.loc[result['speed_error'] > threshold_kmh, 'mistake_type'] = 'TOO_FAST'
...
def generate_insights(df, threshold_kmh):
    ...
    insights.append(f"⚠️  {mistake_count:,} mistakes detected ({mistake_pct:.1f}% of lap)")
    ...
    if too_slow > 0:
        avg_slow_loss = df[df['mistake_type'] == 'TOO_SLOW']['speed_error'].mean()
        insights.append(f"🐌 {too_slow} points too slow (avg {abs(avg_slow_loss):.1f} km/h lost)")
    
    if too_fast > 0:
        avg_fast_excess = df[df['mistake_type'] == 'TOO_FAST']['speed_error'].mean()
        insights.append(f"🏎️  {too_fast} points too fast (avg {avg_fast_excess:.1f} km/h over)")
    
    # Find worst mistakes
    worst_slow = df[df['mistake_type'] == 'TOO_SLOW'].nsmallest(1, 'speed_error')
    worst_fast = df[df['mistake_type'] == 'TOO_FAST'].nlargest(1, 'speed_error')
    
    if len(worst_slow) > 0:
        worst_error = worst_slow.iloc[0]['speed_error']
        insights.append(f"📉 Worst slow mistake: {abs(worst_error):.1f} km/h below expected")
    
    if len(worst_fast) > 0:
        worst_error = worst_fast.iloc[0]['speed_error']
        insights.append(f"📈 Worst fast mistake: {worst_error:.1f} km/h above expected")
    ...
    if 'Steering_Angle' in df.columns:
        ...
        if len(cornering) > 0 and len(straight) > 0:
            ...
            corner_pct = (corner_mistakes / len(cornering)) * 100 if len(cornering) > 0 else 0
            straight_pct = (straight_mistakes / len(straight)) * 100 if len(straight) > 0 else 0
            
            if corner_pct > straight_pct:
                insights.append(f"🔄 More mistakes in corners ({corner_pct:.1f}%) vs straights ({straight_pct:.1f}%)")
            elif straight_pct > corner_pct:
                insights.append(f"➡️  More mistakes on straights ({straight_pct:.1f}%) vs corners ({corner_pct:.1f}%)")
    ...
    if 'pbrake_f' in df.columns and 'pbrake_r' in df.columns:
        ...
        if len(braking) > 0:
            ...
            if braking_pct > total_mistake_pct:
                insights.append(f"🛑 High mistake rate during braking ({braking_pct:.1f}%)")
